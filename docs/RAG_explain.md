...
3. LLM 生成（`explain.py` → `llm_client.query()`）：prompt 模板包含因子表达式、自然语言翻译、真实指标、IC 方向提示、检索到的知识片段，以及解释要求。为避免臆造和夸大，prompt 设有铁律：只能引用检索片段中出现的经典因子公式，无对应时须明说"知识库中无直接对应"；统一指标口径、声明结果为样本内未扣成本；负 IC 必须说明原始因子值与未来收益为反向关系；没有分项贡献或消融证据时，不得断言某一项主导表现。

4. 报告产出：生成 Markdown 报告。每个因子一节，包含表达式、结构翻译、IC 方向提示、指标卡、检索命中的知识来源和 LLM 解释。报告顶部会注明 Top-N 的实际排序口径。
...
| `--topn` | `10` | 取打分前 N 个因子，排序优先级为 `long_excret` → `ic_ir` → `ic_mean` |
